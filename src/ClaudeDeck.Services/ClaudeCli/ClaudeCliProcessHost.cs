...
{
    private readonly ClaudeDeckOptions _options;
    ...
    public ClaudeCliProcessHost(
        IOptions<ClaudeDeckOptions> options,
    ...
    private void StartLocked()
    {
        ...
        _mcpConfigTempPath = Path.Combine(Path.GetTempPath(), $"ClaudeDeck-mcp-{Guid.NewGuid():N}.json");
    ...
    private string BuildArguments(string mcpConfigPath)
    {
        ...
        sb.Append(" --permission-prompt-tool mcp__ClaudeDeck__approval_prompt");
    ...
    {
        // The CLI spawns our helper exe as a stdio MCP server named "ClaudeDeck"
        ...
        var sb = new StringBuilder();
        sb.Append("{\"mcpServers\":{\"ClaudeDeck\":{\"command\":");
        sb.Append(JsonSerializer.Serialize(helperExePath));
        sb.Append(",\"args\":[],\"env\":{\"ClaudeDeck_PERMISSION_PIPE\":");
        sb.Append(JsonSerializer.Serialize(pipeName));
        sb.Append(",\"ClaudeDeck_PERMISSION_SECRET\":");
        sb.Append(JsonSerializer.Serialize(secret));
        sb.Append(",\"ClaudeDeck_PERMISSION_LOG_DIR\":");
    ...
    private string ResolveHelperExePath()
    {
        ...
        // into a per-content-hash subdirectory of %LOCALAPPDATA%\ClaudeDeck\helpers
        ...
        const string ResourceName = "ClaudeDeck-mcp-permissions.zip";
        const string HelperFileName = "ClaudeDeck-mcp-permissions.exe";
