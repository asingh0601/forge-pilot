...
{
    /// <summary>
    /// Claude Deck is a fork and is not published to the VS Marketplace. The
    ...
    /// flipping this flag and filling in a real listing, should Claude Deck ever
    /// be published.
    /// </summary>
    /// <remarks>
    ...
    private const string ExtensionId = "ClaudeDeck.VSExtension.8f4b1c27-6d3a-4e59-9a10-b7c2e5d84f13";
    ...
    public async Task CheckAsync(CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            ...
            Debug.WriteLine($"ClaudeDeck UpdateChecker: {ex.Message}");
        }
    }
    ...
    private static (Version? running, Version? bestOnDisk) ReadVersions()
    {
        ...
        catch (Exception ex)
        {
            ...
            Debug.WriteLine($"ClaudeDeck UpdateChecker: failed to read vsixmanifest: {ex.Message}");
        }
    ...
    private void ShowInfoBar(Version current, Version latest)
    {
        ...
        var model = new InfoBarModel(
            textSpans: new[]
            {
                new InfoBarTextSpan($"ClaudeDeck {latest} is available (you have {current}). "),
            },
            actionItems: new[]
            {
    ...
    private void ShowStaleInstallInfoBar(Version running, Version onDisk)
    {
        ...
        var model = new InfoBarModel(
            ...
            {
                new InfoBarTextSpan(
                    $"ClaudeDeck {onDisk} is installed but Visual Studio is currently running {running}. "
    ...
    public void OnActionItemClicked(IVsInfoBarUIElement infoBarUIElement, IVsInfoBarActionItem actionItem)
    {
        ...
        if (actionItem.ActionContext is string ctx && ctx == "open")
        {
            ...
            {
                Debug.WriteLine($"ClaudeDeck UpdateChecker: failed to open marketplace URL: {ex.Message}");
            }
        }
