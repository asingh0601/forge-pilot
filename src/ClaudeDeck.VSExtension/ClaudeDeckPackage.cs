...
using ClaudeDeck.VSExtension.Options;
using ClaudeDeck.VSExtension.ToolWindows;
...
[ProvideOptionPage(typeof(ClaudeDeckOptionsPage), "ClaudeDeck", "General", 0, 0, true)]
...
public sealed class ClaudeDeckPackage : AsyncPackage, IVsSolutionEvents
{
    private static ClaudeDeckPackage? _instance;
    ...
    private async Task InitializeSessionPersistenceAsync()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"ClaudeDeck: Failed to initialize session persistence: {ex}");
        }
    }
    ...
    private async Task PurgeOldSessionsAsync()
    {
        ...
        try
        {
            var optionsPage = (ClaudeDeckOptionsPage?)GetDialogPage(typeof(ClaudeDeckOptionsPage));
            var keepDays = optionsPage?.KeepActivityDays ?? 30;
            await _sessionStore.DeleteSessionsOlderThanAsync(_solutionDirectory, keepDays);
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"ClaudeDeck: Failed to purge old sessions: {ex}");
        }
    }
    ...
    private ChatSessionViewModel CreateChatViewModel()
    {
        ...
        var logPath = Path.Combine(
            ...
            "ClaudeDeck", "logs", "ClaudeDeck-.log");
        ...
        // Read persisted settings from Tools → Options → ClaudeDeck
        var optionsPage = (ClaudeDeckOptionsPage?)GetDialogPage(typeof(ClaudeDeckOptionsPage));
        ...
        var optionsAccessor = provider.GetRequiredService<Microsoft.Extensions.Options.IOptions<ClaudeDeck.Services.Configuration.ClaudeDeckOptions>>();
        var permissionBroker = provider.GetRequiredService<ClaudeDeck.Services.ClaudeCli.Permissions.IPermissionBroker>();
        var questionBroker = provider.GetRequiredService<ClaudeDeck.Services.ClaudeCli.Questions.IUserQuestionBroker>();
    ...
    private static async Task OpenOrActivateSessionAsync(SessionInfo session)
    {
        ...
        if (_instance._sessionWindowMap.TryGetValue(session.Id, out int existingWindowId))
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"ClaudeDeck: Failed to activate session window: {ex}");
            }
            return;
        }
                ...
                if (window is ChatSessionToolWindow chatWindow)
                {
                    ...
                    catch (Exception ex)
                    {
                        viewModel = new ChatSessionViewModel(
                            Environment.GetFolderPath(Environment.SpecialFolder.UserProfile));
                        System.Diagnostics.Debug.WriteLine($"ClaudeDeck: Failed to create chat service: {ex}");
                        MessageBox.Show($"ClaudeDeck service init failed:\n\n{ex.Message}\n\n{ex.InnerException?.Message}", "ClaudeDeck Debug", MessageBoxButton.OK, MessageBoxImage.Warning);
                    }
        ...
        {
            MessageBox.Show($"ClaudeDeck error: {ex.Message}", "ClaudeDeck", MessageBoxButton.OK, MessageBoxImage.Error);
        }
        finally
        {
    ...
    private async Task SwitchWorkspaceAsync(string newSolutionDirectory)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"ClaudeDeck: Failed to switch workspace: {ex}");
        }
    }
    ...
    private void OnFileOpenRequested(string rawPath)
    {
        _ = JoinableTaskFactory.RunAsync(async () =>
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"ClaudeDeck: File not found: {filePath}");
                return;
            }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"ClaudeDeck: Failed to open file: {ex.Message}");
            }
        });
    }
