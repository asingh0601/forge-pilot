...
public sealed class ClaudeCliProcessHost : IDisposable
{
    ...
    private void StartLocked()
    {
        ...
        _mcpConfigTempPath = Path.Combine(Path.GetTempPath(), $"ForgePilot-mcp-{Guid.NewGuid():N}.json");
    ...
    private string BuildArguments(string mcpConfigPath)
    {
        ...
        sb.Append(" --permission-prompt-tool mcp__ForgePilot__approval_prompt");
    ...
    {
        // The CLI spawns our helper exe as a stdio MCP server named "ForgePilot"
        ...
        var sb = new StringBuilder();
        sb.Append("{\"mcpServers\":{\"ForgePilot\":{\"command\":");
        sb.Append(JsonSerializer.Serialize(helperExePath));
        sb.Append(",\"args\":[],\"env\":{\"ForgePilot_PERMISSION_PIPE\":");
        sb.Append(JsonSerializer.Serialize(pipeName));
        sb.Append(",\"ForgePilot_PERMISSION_SECRET\":");
        sb.Append(JsonSerializer.Serialize(secret));
        sb.Append(",\"ForgePilot_PERMISSION_LOG_DIR\":");
    ...
    private string ResolveHelperExePath()
    {
        ...
        // into a per-content-hash subdirectory of %LOCALAPPDATA%\ForgePilot\helpers
        ...
        const string ResourceName = "ForgePilot-mcp-permissions.zip";
        const string HelperFileName = "ForgePilot-mcp-permissions.exe";
