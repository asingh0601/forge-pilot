...
namespace ForgePilot.UI.Controls;
...
public partial class ChatWebView : UserControl
{
    ...
    private static string LoadHtmlTemplate()
    {
        ...
        var showdownJs = ReadEmbeddedResource(assembly, "ForgePilot.UI.Assets.showdown.min.js");
        var templateHtml = ReadEmbeddedResource(assembly, "ForgePilot.UI.Assets.chat-template.html");
    ...
    private async void OnNavigationCompleted(object? sender, CoreWebView2NavigationCompletedEventArgs e)
    {
        ...
        // Push the palette before replaying queued messages, so restored
        // transcripts never paint with the fallback colours first.
    ...
    /// <summary>
    /// Sets the light/dark variant, then overwrites the individual custom
    /// properties from the live WPF dictionary. Both are needed: the variant
    /// attribute drives rules the host doesn't push (radii, shadows), while the
    /// explicit colours keep the transcript identical to the chrome around it.
    /// </summary>
    public async Task ApplyCurrentThemeAsync()
    {
        var variant = ClaudeThemeManager.Current.ToString();
        await ExecuteOrQueueAsync($"setTheme({JsonSerializer.Serialize(variant)})");
        await SetThemeColorsAsync(ClaudeThemeManager.ToCssVariables());
    }
