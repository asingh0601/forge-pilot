...
namespace ForgePilot.Services.McpPermissionServer;

/// <summary>
/// Tiny stdio MCP server that exposes a single tool, <c>approval_prompt</c>,
/// for use with <c>claude --permission-prompt-tool mcp__ForgePilot__approval_prompt</c>.
///
/// When the CLI calls the tool, this process forwards the request over a
/// named pipe (set via <c>ForgePilot_PERMISSION_PIPE</c> env var, with the
/// shared secret in <c>ForgePilot_PERMISSION_SECRET</c>) to the parent
/// ForgePilot extension, which surfaces it to the user and replies with
/// allow/deny.
///
...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        try
        {
            var logDir = Environment.GetEnvironmentVariable("ForgePilot_PERMISSION_LOG_DIR");
            if (string.IsNullOrEmpty(logDir))
                logDir = Path.GetTempPath();
            _logFilePath = Path.Combine(logDir, $"ForgePilot-mcp-helper-{DateTime.Now:yyyyMMdd}.log");
        }
        catch { }
        ...
        try
        {
            var pipeName = Environment.GetEnvironmentVariable("ForgePilot_PERMISSION_PIPE");
            var secret = Environment.GetEnvironmentVariable("ForgePilot_PERMISSION_SECRET");
            Log($"ForgePilot_PERMISSION_PIPE={pipeName}");
            Log($"ForgePilot_PERMISSION_SECRET={(string.IsNullOrEmpty(secret) ? "(empty)" : "***")}");

            if (string.IsNullOrEmpty(pipeName) || string.IsNullOrEmpty(secret))
            {
                Log("ERROR: env vars missing, exiting with code 2");
                Console.Error.WriteLine("ForgePilot_PERMISSION_PIPE / _SECRET env vars missing");
                return 2;
            }
    ...
    private static string BuildInitializeResult()
    {
        ...
        sb.Append(",\"serverInfo\":{\"name\":\"ForgePilot-permissions\",\"version\":\"1.0.0\"}}");
        return sb.ToString();
    }
